...
18. **不允许中途换 accent 色**——4 套主题（薄荷 / 青墨 / 暖橙 / 深空）选一个用到底（见 `themes-corp.md`）。
...
19. **`.qjyd-img-placeholder` 只在"该页天然需要图但素材未到位"时使用，禁止当作"找不到合适版式的兜底"**。
    ...
    **正确做法**：
    ...
    - 内容是"成果 / 瓶颈" / "已完成 / 待完成" / "亮点 / 不足" → 用 **C15 Status Duo**
...
##### 双轨网格: 标题永远画布对齐, 主内容按版式自由 (**新增 · 2026-06 · 更新十八, 修订更新十七**)

24. **同一份 deck 里, 标题左缘 (H1 + meta) 必须始终对齐画布左缘 (qjyd-corp 是 x=88, 即 section padding-left)**, 与具体版式无关。主内容容器可以根据版式语义自由选择"画布左对齐"或"整组居中", 但标题不参与这个变化。

    **判定**:
    - 单栏 / 主从 / 列表 (C04 / C06 / C16 / C07 …) → H1 + 主内容都在 x=88 ✅
    - 双栏对照 / 平行结构 (C15) → H1 在 x=88, 主内容整组居中 x≈212 ✅ (标题与内容左缘故意错位 ~120 px)
    - **不允许**: 让 H1 跟随主内容居中 (像 v17 那样把标题也限宽 1177 居中) → 翻页时标题位置忽左忽右, 失去锚点

    **错位幅度的硬性档位** (这是这一轮反复踩出来的经验):

    | 错位 | 视觉读法 | 何时用 |
    |---|---|---|
    | 0 ~ 2 px | 完全对齐 | 标题跟主内容用同一网格的页 (C04 等) |
    | **3 ~ 60 px** | **bug, 像没对齐** ❌ | **禁区**, 不要落在这个区间 |
    | 60 ~ 80 px | 摇摆区 ⚠️ | 可避免就避免 |
    | **≥ 80 px** | 有意的版式错位 ✅ | 标题画布对齐 + 主内容居中 (典型 ~120 px) |

    踩过的坑 (这条红线的形成史):
    - **更新十六**: status-duo 居中 (left=212), 标题区 max-width:1128 居中 (left=236), 错位 24 px → 直接被视觉模型读成 bug
    - **更新十七**: 改用共享限宽变量, H1 也居中到 left=212, 错位 0 px → 用户翻页对比立刻发现 "C15 标题位置跟其他页不一样"
    - **更新十八 (定稿)**: H1 还原 x=88 (与其他所有页一致), status-duo 保持居中 x=212, 故意错位 124 px → 视觉模型评价 "更偏向有版式意图, 不像 bug; 如果其他页面 H1 都在这个左缘, 那这页是成立的"

    **核心原则**: 跨页一致性 > 单页自洽。在 ≥ 80% 的页都用某一种网格的 deck 里, 标题永远跟随大多数派, 让少数派的主内容用自己的网格, 通过 ≥ 80 px 的明确错位表达版式差异。完整推导见 `references/layouts-corp.md` 「2026-06 更新十八」。

    **CSS 落点**:
    - 把"主内容总宽"做成 section 内联 CSS 变量: `<section ... style="--c15-track-w:1177px">` (作用域只在这一页)
    - 主内容容器: `max-width:var(--c15-track-w); margin:0 auto; grid-template-columns:1fr 1px 1fr;`
    - **标题容器: 不要套这个变量, 不要 margin:auto**——让它跟随 section 的 padding 自然贴齐画布左缘
...
| 6 | **想要哪套主题色?** | 杂志风 5 套(`themes.md`) / 瑞士风 4 套(`themes-swiss.md`) / 企业风 4 套(`themes-corp.md`),挑一 |
...
| 内容是"本周做了 N 件事"/"亮点 vs 不足"/"已完成 vs 待完成" | **C 更合适**(C16 / C15 直接对应,无需图片) |
...
| 2 列对照（成果/瓶颈、本周/下周、问题/方案、A/B 方案、新/旧） | C15 Status Duo |
