...
| C15 | 双栏对照 | `status-duo` | **恰 2 列** | 否 | 左右两栏（成果/瓶颈、本/下周、问题/方案均可） |
...
**用途**：任何"两栏并列对照"的场景——成果 vs 瓶颈、本周 vs 下周、问题 vs 方案、旧方案 vs 新方案、A 方案 vs B 方案、已完成 vs 待完成、亮点 vs 不足。两栏 1:1 等宽，中间 hairline 分隔，**整页无图**。

**核心设计原则**：

- **左栏用 `.done` = accent 海洋青（"正向 / 已发生"语义） · 右栏用 `.next` = 中性灰（"下一步 / 待发生"语义）**
- ❌ 不要用红色给"瓶颈/不足"上色——会变成"出问题告警"，但绝大多数"瓶颈"和"下一步"是中性的，不是 alarm
- 左栏 dot ●（accent 实心方点带 4px 光晕） · 右栏 dot ○（空心圆环）
- 如果你的内容是「方案 A vs 方案 B」这种**等权对照**，两栏都用 `.done` 也行（视觉上变成"双 accent"），看你想不想暗示一边更优
...
| ✅ 适用（任何 2 栏等权对照） | ❌ 不适用 |
|---|---|
| 周报"本周做了 vs 下周做" / 复盘"亮点 vs 不足" | 三栏对比（用 C11） |
| 项目"已交付 vs 待交付" / 技术债"已解决 vs 待解决" | 历史路径（用 C06 timeline） |
| 方案"旧 vs 新" / "竞品 vs 我们" | 数字大字报（用 C04） |
| 调研"问题 vs 方案" / 客户"现状 vs 目标" | 4 项并列（用 C07 / C16 `.cols-2`） |

**常见错误**：

1. ❌ 用红色给 NEXT 栏上色——告警语义太重。中性灰才是 qjyd-corp 的"下一步"色
2. ❌ 左右栏内容数量差距过大（左 2 条 vs 右 6 条）——视觉失衡。如果素材天然不平衡，**改用 C14 dense + 一段说明**，而不是硬塞成 duo
3. ❌ 给 `.qjyd-status-duo` 加 `align-items: stretch`——左右栏会被强制等高，sd-points 短的那栏中间会出现大段留白。
4. ❌ 给 `.sd-foot` 加 `margin-top: auto`——会把 sd-foot 推到页面底部，但左栏没有 sd-foot 时双栏不对称。**保持紧跟 sd-points 之后**

**`align-items` 的取舍**（2026-06 更新十四纠正）：

> 模板早期 (更新九) 用 `align-items: center` 解决"两栏都很短时底部留白大"；但**当两栏 bullet 数量不一致时（比如左 3 右 4），center 会让短栏整体往下压 60+ px，每行元素（tag/title/badge/bullet）左右错位，视觉上极别扭**。

| 场景 | 用 |
|---|---|
| 两栏内容长度相近（差 ≤ 1 行）| 任意（center 视觉略居中、start 上对齐都可）|
| **两栏内容长度差距明显**（差 ≥ 2 行 或 一栏有 sd-foot 另一栏没有）| ✅ **必须 `align-items: start`**，并主动给短的那栏补 `sd-foot` 让两栏底部也接近齐平 |
| 两栏都用了 `sd-headline` + 徽章 | ✅ `align-items: start`（保证 tag/title/badge 行行对齐）|

**模板默认值已改为 `align-items: start`**（更新十四）。如果出现"短栏底部空一截"的问题，**优先补 sd-foot 内容平衡**，不要为此改回 center 牺牲行对齐。
...
| 2 项左右对照 | C15 双栏 | 中间有 hairline，左右栏色彩语义可分 |
...
| C15 双栏对照 / C11 数据对比表 | `var(--sp-7)` | 32 |
...
## 2026-06 更新十八: C15 标题归位画布左缘 (双轨网格定稿)
