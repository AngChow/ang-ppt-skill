...
1. ✅ 数字型 KPI 默认使用单行 `.kpi-num`（已加 `white-space:nowrap` 防单位换行），单位用 `.kpi-sfx`（`vertical-align:baseline` 底部锚定 + `margin-left:.24em` 留呼吸）。
2. ✅ 概念型 KPI 使用 `.kpi-num.kpi-num-stack` + `.kpi-main` / `.kpi-sfx` 两段式结构；stack 会比默认数字型 KPI 略小一档，保留冲击力但避免两行文字压迫卡片留白。
3. ✅ 同一页三项必须统一：要么全部单行数字型，要么全部 stack 两行概念型。**数字型 KPI 内部也不允许换行** — `white-space:nowrap` 从 CSS 层兜底, 确保 "19,714条" 不会拆成两行 (详见更新二十七 · 红线 43)。
...
## 2026-06 更新二十七 · C05 数字型 KPI 单行对齐修复 (红线 43-45)

### 现象

C05 三段 KPI 页, 三个数字型 KPI (单行 `.kpi-num` + `.kpi-sfx`) 在真实数据下出现:

1. **数字 + 中文单位被拆成两行**: "19,714" 一行, "条" 掉到第二行; "14,456" 一行, "次" 掉到第二行。只有 "93%" 因为短而保持单行。
2. **三个卡片大数字高度不一致**: 第一个卡片数字 1 行, 后两个卡片数字 2 行, 视觉重心参差。
3. **单位符号 `vertical-align:.55em` 浮在大字中上部**: 像上标, 视觉上 "悬空", 底部没有锚定感。
4. **`.kpi-label` 行数不一致时底部基线参差**: 单行 label 和两行 label 底部不齐, 下方 desc 起始位置跟着错位。

### 根因

| 问题 | 根因 |
|---|---|
| 单位换行 | `.kpi-num` 未设 `white-space:nowrap`; 数字 + 中文单位在卡片宽度不够时浏览器允许换行 |
| 字号过大 | `clamp(72px, 7.2vw, 132px)` 在 1600 宽下 ~115px, "19,714条" 宽度超出卡片内容区 (~373px) |
| 单位悬空 | `vertical-align:.55em` 把单位符号基线抬高 0.55em, 配合 `.4em` 字号, 单位浮在大字中上部 |
| label 参差 | `.kpi-label` 无 `min-height`, 行数由文字自然换行决定, 底部基线不统一 |

### 修复 (模板 + PPT 同步)

```css
/* 1. 数字不换行 + 字号降一档 */
.qjyd-kpi-triple .kpi-num{
  font-size:clamp(64px, 6.4vw, 112px);  /* 原 clamp(72px, 7.2vw, 132px) */
  white-space:nowrap;                    /* 新增: 阻止数字与单位之间换行 */
}

/* 2. 单位贴底 + 间距加大 */
.qjyd-kpi-triple .kpi-num .kpi-sfx{
  vertical-align:baseline;               /* 原 .55em (上标式悬浮) → baseline (底部锚定) */
  margin-left:.24em;                     /* 原 .06em → .24em (数字与单位之间留呼吸) */
}

/* 3. label 统一两行高度 + 贴底 */
.qjyd-kpi-triple .kpi-label{
  margin-top:var(--sp-4);                /* 原 sp-2 → sp-4 (与数字拉开) */
  min-height:2.6em;                      /* 新增: 强制两行高度, 单行也占两行 */
  display:flex; align-items:flex-end;    /* 新增: 文字贴底, 底部基线统一 */
}
```

### 红线

43. **`.kpi-num` 必须设 `white-space:nowrap`** — 数字型 KPI 的数字 + 单位是一个不可分割的整体, 浏览器默认允许在数字和中文单位之间换行, 会导致 "19,714" 和 "条" 分成两行, 破坏单行节奏。任何去掉 `nowrap` 的改动都视为 regression。

44. **数字型 KPI 的单位 `vertical-align` 默认 `baseline`** — `.55em` 上标式悬浮只适合纯英文场景 (如 `100%`); 中文单位 ("条"/"次"/"家") 用 baseline 底部锚定更稳重。如需恢复上标风格, 只在纯英文/数字单位的场景局部覆盖。

45. **`.kpi-label` 必须设 `min-height` + `align-items:flex-end`** — 三个卡片的 label 文字长度不同时 (如 "独立承接率" 5 字 vs "Chat 有效回答" 中英混合), 行数会不一致。`min-height:2.6em` 强制统一两行高度, `align-items:flex-end` 让文字贴底, 保证三个 label 底部基线对齐, 下方 desc 起始位置自然统一。

### 经验

- **数字型 KPI 的 "单行" 不只是视觉偏好, 是结构约束**: 当三个 KPI 都是数字型 (单行 `.kpi-num`) 时, 任一个的数字+单位换行就会变成 "两行", 与其他两个 "单行" 的高度不一致 — 这和规则 #3 (同一页三项必须统一) 是同一原则的不同表现。`white-space:nowrap` 从 CSS 层兜底, 确保数字型 KPI 不会因数据长度意外 "退化" 成两行。
- **`vertical-align` 的选择跟单位语言相关**: 英文单位 (`%`, `+`) 适合上标 (`.55em`), 因为字符本身有 ascender, 视觉上浮在中上部时与数字形成 "数字为主、单位为辅" 的层次; 中文单位 ("条"/"次"/"家") 是方块字, 上标式悬浮会让它 "悬空", 用 `baseline` 底部锚定更自然。
- **`min-height` + `align-items:flex-end` 是 label 对齐的经典兜底**: 不需要改文字内容, 只靠 CSS 就能让不同长度的 label 底部齐平。代价是单行 label 上方多一块留白, 但因为文字贴底, 留白在上方 (靠近数字), 视觉上相当于 "数字和 label 之间多了一点呼吸", 可以接受。
