...
| 主题 | 状态 | 说明 |
|---|---|---|
| 🌿 **1 薄荷企业** | ✅ **已调优, 生产可用** | 默认主题, 各版式视觉细节都已经过多轮迭代 |
| 🌊 2 青墨科技 | ⚠️ **样式细节尚未充分调优, 暂不建议生产使用** | 主色已定, 但深底卡片 / chip / accent 灰阶尚未跟版式逐页调试 |
| 🌅 3 暖橙提案 | ⚠️ **样式细节尚未充分调优, 暂不建议生产使用** | 同上 |
| 🌌 4 深空述职 | ⚠️ **样式细节尚未充分调优, 暂不建议生产使用** | 反色 dark 主题, 16 个版式还没逐页验证浅 accent / 卡片层级在 dark 底上的视觉一致性 |

**默认行为**:
- 用户**没有明确指定**主题色 → **直接用 🌿 1 薄荷企业**, 不需要在决策表里到处对照
- 用户**明确说**"我要青墨 / 暖橙 / 深空" → 用对应主题, 但要在回复里**主动告知**"该主题样式细节尚未充分调优, 部分版式可能不如默认薄荷成熟"
- 不要因为内容看着"是数据汇报 / 是销售提案 / 是述职"就**主动**推荐非默认主题——决策表是"用户已经明确说要换主题"时才查的对照表, 不是"主动推荐表"
...
## 主题与场景的语义匹配 (仅在用户明确要换主题时查)

> ⚠️ 这张表**不是"主动推荐表"**, 是"用户明确说要换主题时, 该选哪一套"的对照表。
> 在用户没有明确要求换主题时, 一律默认 🌿 1 薄荷企业 (见上方"主题成熟度状态")。

| 用户场景 | 语义匹配 | 当前状态 |
|---|---|---|
| 默认 / 不知道选啥 / 通用商务提案 / 周报 / 月报 / 复盘 / 团队同步 | **🌿 1 薄荷企业** | ✅ 已调优 |
| 技术分享 / 数据汇报 / 研发对外发布 | 🌊 2 青墨科技 | ⚠️ 暂不建议生产用 |
| 销售提案 / 增长汇报 / 营销活动 | 🌅 3 暖橙提案 | ⚠️ 暂不建议生产用 |
| 职级评审 / 个人述职 / 团队复盘 / 年终总结 | 🌌 4 深空述职 | ⚠️ 暂不建议生产用 |
...
6. **默认薄荷, 不主动推荐其他三套** — 青墨 / 暖橙 / 深空当前样式细节尚未充分调优, 用户**没有明确要求换主题**时一律用 🌿 1 薄荷企业; 即使内容看上去"很数据汇报"或"很销售提案", 也不要主动建议换主题。只在用户明确说出主题名时才切换, 并要主动告知该主题尚未充分调优
